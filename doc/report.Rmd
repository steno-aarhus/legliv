...
the 2017 UK Physical activity guidelines of 150 minutes of moderate
activity per week or 75 minutes of vigorous activity, or missing data
[categorical]), and smoking (pack years as a proportion of lifespan
exposed to smoking [continous])

categorical), and alcohol intake (grams/day continuous).
Sociodemographic and anthropometric confounders were selected from the
initial assessment visit before the start of follow-up.
...
15 grams of total meat, red meat, or processed meat per day (legumes for
total meat: 0.98 (0.93, 1.04), legumes for red meat: 0.97 (0.91-1.03),
legumes for processed meat: 1.02 (0.93, 1.13). the results did not
change when with further adjustments (legumes for total meat: 1.02
(0.96, 1.08), legumes for red meat: 1.00 (0.94-1.07), legumes for
processed meat: 1.09 (0.98, 1.20).

Estimates seemed to go in opposite directions when substituting total
meat or red meat with legumes for risk of Liver cell carcinoma (total
meat: 1.08 (0.99, 1.19), red meat: 1,07 (0.97, 1.18)) and intrahepatic
bile duct carcinoma (total meat: 0.97 (0.90, 1.05), red meat: 0.95
(0.87, 1.03), but these trends were not statistically significant
(supplementary table 1). Further, excluding high alcohol consumers or
food intake misreporters or setting inclusion criteria to ≥ 3 completed
diet questionnaires did not alter the estimates in any statistically
significant way (supplementary tables 3 to 5). In an adjusted no
substitution model, a mean intake of 6.3 grams of legumes per day was
associated with a reduced risk of liver cancer, compared to no intake
(0.59 (0.35, 0.98)); however, no associations were observed with further
increase in legume intake (supplementary table 6).
...
table_combined <- tbl_merge(
  ...
  tab_spanner = c("**All participants**", "**Participants who developed liver cancer**")
) %>%
  bold_labels() %>%
...
food_group <- c(
  ...
  "Vegetable spread", "Tea, coffee & water", "Coffee, caffeinated",
...
df <- data.frame(food_group = food_group_df,
                 includes = includes,
                 ukb_biobank_id = ukb_id_number_df)
...
food_group_table <- df %>%
  ...
  cols_label(
    ...
    uk_bio_bank_id = md("**UK Biobank ID**")
  ) %>% 
  tab_style(
...
table_cancer_type <-
  ...
  modify_caption("**Supplementary table 2. Hazard ratios for substitution of legumes for total meat, red meat and processed meat for each cancer type.** (N = {N})") %>%
  modify_header(label = "**15 g/day substitution**") %>%
  modify_footnote(update = everything() ~ NA, abbreviation = T) %>%
  as_gt() %>%
  tab_spanner(
